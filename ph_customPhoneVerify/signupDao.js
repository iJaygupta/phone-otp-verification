...
const cryptoRandomString = require('/opt/node_modules/crypto-random-string');
const uuidv1 = require('/opt/node_modules/uuid/v1');
...
const usersBlockTableSchema = {
  tableName: "users_block_ph",
  partitionKeyName: "id",
  sortKeyName:"dateTime"
}
...
class SignupDao  {
    ...
    prepareParamsToGetOTP(userName, globalTimeFormat) {
        let peQuery = new ParamExpressionQuery();
    ...
    prepareParamsToQueryUserPhoneNumber(phoneNumber) {
     ...
     prepareParamsForPutUserBlockData(userName,dateTime,blockTill,blockType,statusCode,errorCode){
        let pePut = new ParamExpressionPut();
        pePut.tableName = usersBlockTableSchema.tableName;
        pePut.payload = {
          [usersBlockTableSchema.partitionKeyName]: userName,
          [usersBlockTableSchema.sortKeyName]: dateTime,
          blockTill: blockTill,
          autoBlockTill: blockTill,
          blockType: blockType,
